...
function AdminTryoutsPage() {
  ...
  const editTryout = (tryout: TryoutRow) => {
    setForm({
      id: tryout.id,
      title: tryout.title,
      description: tryout.description,
      categoryId: tryout.categoryId,
      durationMinutes: String(tryout.durationMinutes),
  ...
  const saveTryout = async () => {
    ...
    try {
      if (isEditing) {
        await updateTryoutAdmin({ data: { ...payload, tryoutId: form.id } });
      } else {
  ...
  const setPublication = async (tryoutId: string, nextStatus: "published" | "unpublished") => {
    setBusyAction(`publish:${tryoutId}`);
    setErrorMessage("");
  ...
  const downloadWorkbook = async (tryoutId: string) => {
    ...
    try {
      ...
      saveWorkbook(XLSX, workbook, `${workbookData.tryout.slug || "tryout"}-workbook.xlsx`);
    } catch {
  ...
  const importWorkbook = async (tryoutId: string, file: File | undefined) => {
    if (!file) return;

    setBusyAction(`import:${tryoutId}`);
    setErrorMessage("");
  ...
  const downloadSampleWorkbook = async () => {
    ...
    try {
      ...
      saveWorkbook(XLSX, workbook, "ilmorax-tryout-sample.xlsx");
    } catch {
      ...
      <div className="admin-lane">
        ...
        <section className="admin-panel mt-6">
          <div className="admin-panel-header">
            <h2 className="admin-panel-title">{isEditing ? "Edit Try-out" : "Create Try-out"}</h2>
          </div>
          ...
          <div className="grid gap-5 p-5 sm:p-6">
            ...
            <div className="flex flex-wrap gap-3 pt-1">
              <button
                ...
                type="button"
              >
                {isEditing ? "Save changes" : "Create Try-out"}
              </button>
              {isEditing && (
                <button
                  onClick={resetForm}
              ...
              <div key={tryout.id} className="admin-list-row">
                ...
                <div className="admin-list-actions">
                  ...
                  <div className="admin-list-actions-bar">
                    ...
                    <PublicationButton
                      tryout={tryout}
                      busy={busyAction === `publish:${tryout.id}`}
                    ...
                    <FileUpload
                      accept=".xlsx"
                      busy={busyAction === `import:${tryout.id}`}
                      compact
                      placeholder="Import"
                      onFileSelect={(file) => importWorkbook(tryout.id, file)}
                    />
                  </div>
                </div>
              </div>
...
function PublicationButton({
  ...
  tryout: TryoutRow;
  busy: boolean;
  onChange: (tryoutId: string, nextStatus: "published" | "unpublished") => void;
}) {
  if (tryout.status === "published") {
    return (
      <button
        onClick={() => onChange(tryout.id, "unpublished")}
        disabled={busy}
  ...
  return (
    <button
      onClick={() => onChange(tryout.id, "published")}
      disabled={busy}
