...
import type { listLeaderboard } from "./student-leaderboard-functions";
...
export function LeaderboardPage({ leaderboard, summary }: LeaderboardPageData) {
  const leaderboardUsers = toLeaderboardEntryViews(leaderboard.entries);
  const podium = [
    leaderboardUsers[1],
    leaderboardUsers[0],
    leaderboardUsers[2],
  ...
  const leaderGap = getLeaderboardLeaderGap(viewerEntry, leader);
  const weekRange = formatLeaderboardWeekRange(leaderboard.week.startsAt, leaderboard.week.endsAt);
  const rewardsAt = formatLeaderboardRewardsAt(leaderboard.week.rewardsFinaliseAt);
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[13.5px] leading-relaxed text-stone-500 font-medium max-w-[30ch]">
                    Papan peringkat ini ditutup {formatLeaderboardRewardsAt(leaderboard.week.endsAt)}.
...
function PodiumCard({ user }: { user: LeaderboardEntryView }) {
...
function LeaderboardRow({ user }: { user: LeaderboardEntryView }) {
